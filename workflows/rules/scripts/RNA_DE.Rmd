...
metadf$condition <- factor(metadf$condition, levels = c('wt', 'dell1', 'dell2', 'deln', 'msl1null'))
metadf
```
...
WT_vs_dell1 <- lfcShrink(dds, contrast=c(0,1,0,0,0), type="ashr") %>% mergeres
WT_vs_dell2 <- lfcShrink(dds, contrast=c(0,0,1,0,0), type="ashr") %>% mergeres
WT_vs_deln <- lfcShrink(dds, contrast=c(0,0,0,1,0), type="ashr") %>% mergeres
WT_vs_MSL1null <- lfcShrink(dds, contrast=c(0,0,0,0,1), type="ashr") %>% mergeres
...
scale_rows = function(x){
    ...
    finscores <- cbind(
      WT_MSL1, delL1, delL2, delN, MSL1NULL
    ) %>% data.frame
    return(finscores)
}
Zscores <- scale_rows(
...
g <- ggplot(data=df) +
  ...
  geom_violin(alpha =0.4, width=1, aes(x=samples, fill=samples, y=zscores)) +
  #geom_boxplot(width=0.1, aes(x=samples, y=zscores)) +
  scale_fill_manual(values = c("#1f77b4", "#7f7f7f", "#7f7f7f", "#7f7f7f", "#1f77b4")) +
  theme(legend.position = "none") +
  theme_minimal() +
...
# ))
df$sample_chrom <- factor(df$sample_chrom, levels=c(
  'WT_MSL1|X', 'delL1|X', 'delL2|X', 'delN|X', 'MSL1NULL|X',
  'WT_MSL1|non-X', 'delL1|non-X', 'delL2|non-X', 'delN|non-X', 'MSL1NULL|non-X'
))
